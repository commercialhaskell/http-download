...
import           Network.HTTP.Client ( HttpException, Request, getUri, path )
...
import           RIO.PrettyPrint ( HasTerm, flow, prettyWarn, vcat )
...
mkDownloadRequest req =
  DownloadRequest req [] Nothing drRetryPolicyDefault False

-- | Modify the 'Request' inside a 'DownloadRequest'. Especially intended for
...
drRetryPolicyDefault =
  limitRetries 7 <> exponentialBackoff onehundredMilliseconds
 where
...
displayCheckHexDigest (CheckHexDigestByteString s) =
  displayByteString s ++ " (ByteString)"
...
recoveringHttp retryPolicy =
 ...
 where
  ...
  helper wrapper action =
    withUnliftIO $ \run -> wrapper run (unliftIO run action)
...
verifiedDownload DownloadRequest{..} destpath progressSink = do
  ...
  whenM' (liftIO getShouldDownload) $ do
    logDebug $
      "Downloading " <> display (decodeUtf8With lenientDecode (path req))
 ...
 where
  whenM' mp m = do
    p <- mp
    if p then m >> pure True else pure False

  fp = toFilePath destpath
  dir = toFilePath $ parent destpath

  getShouldDownload = if drForceDownload
    then pure True
    else do
  ...
  fileMatchesExpectations =
    ((checkExpectations >> pure True)
      `catch` \(_ :: VerifyFileException) -> pure False)
      `catch` \(_ :: VerifiedDownloadException) -> pure False
  ...
  go sink res = do
    ...
    let hashChecks =
             ( case List.lookup hContentMD5 headers of
                 Just md5BS ->
                   [ HashCheck
    ...
    maybe id (\len -> (CB.isolate len .|)) drLengthCheck $ getZipSink
